...
\begin{itemize}[leftmargin=*,itemsep=0.5em]
    \item \textbf{Classical probabilistic data structures:} Bloom filters~\cite{bloom1970space}, counting Bloom filters, SimHash~\cite{charikar2002similarity}, and suffix arrays~\cite{manber1993suffix} are widely used for approximate membership, similarity, and substring search.
...
We prove bounds on false positive rates for QAM, analyze noise robustness, and discuss the variance reduction from quantum batching. Key results:
\begin{itemize}[leftmargin=*,itemsep=0.5em]
    \item \textbf{QAM false-positive bound:} $\alpha \leq \exp(-C k (1-\rho))$ for load factor $\rho = |S|/m$.
    \item \textbf{Noise perturbation:} Acceptance gap degrades $\leq O(k\epsilon)$ under Pauli noise.
...
Current quantum hardware is limited in qubit count and noise. Simulation is memory-intensive. Open questions include error mitigation, dynamic/streaming QDS, and hardware experiments.
